...
public class Skill10 : MonoBehaviour
{
    ...
    void Use()
    {
        if (Input.GetKeyDown(SkillCode) && !IsSkill && ph.MP >= sm.skills[10].consumeMp)
        {
            if (!IsSkill)
            {
            ...
            rangeObj.transform.localScale = new Vector3(sm.skills[10].distance, sm.skills[10].distance, sm.skills[10].distance);
            skillrangeObj.transform.localScale = new Vector3(sm.skills[10].explo, sm.skills[10].explo, sm.skills[10].explo);
        ...
        if (IsSkill)
        {
            ...
            if (Physics.Raycast(cam.ScreenPointToRay(Input.mousePosition), out hit, 100, mask))
            {
                ...
                float a, b;
                if (dis <= sm.skills[10].distance * 0.4f)
                {
                ...
                {
                    a = sm.skills[10].distance * 0.4f * (hit.point.x - pm.transform.position.x) / dis + pm.transform.position.x;
                    b = sm.skills[10].distance * 0.4f * (hit.point.z - pm.transform.position.z) / dis + pm.transform.position.z;
                    if (skillrangeObj != null)
                ...
                {
                    ph.MP -= sm.skills[10].consumeMp;
                    sound.sounds[11].Play();
                    isTimer = true;
                    skillObj = Instantiate(skillPrefab, skillrangeObj.transform.position, Quaternion.identity);
                    skillObj.transform.localScale = new Vector3(0.2f + sm.skills[10].explo * 0.005f, 0.2f + sm.skills[10].explo * 0.005f, 0.2f + sm.skills[10].explo * 0.005f);
                    Destroy(skillObj, sm.skills[10].sustaintime);
        ...
        if (isTimer)
        {
            float cooltime = sm.skills[10].cooltime * (1 - 0.01f * status.Calcool);
