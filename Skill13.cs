...
public class Skill13 : MonoBehaviour
{
    ...
    void Use()
    {
        if (Input.GetKeyDown(SkillCode) && !IsSkill && ph.MP >= sm.skills[13].consumeMp)
        {
            ...
            rangeObj.transform.localScale = new Vector3(sm.skills[13].distance, sm.skills[13].distance, sm.skills[13].distance);
            skillrangeObj.transform.localScale = new Vector3(sm.skills[13].explo * 1.7f, sm.skills[13].explo * 1.7f, sm.skills[13].explo * 1.7f);
        ...
        if (IsSkill)
        {
            ...
            if (Physics.Raycast(cam.ScreenPointToRay(Input.mousePosition), out hit, 100, mask))
            {
                ...
                float a, b;
                if (dis <= sm.skills[13].distance * 0.4f)
                {
                ...
                {
                    a = sm.skills[13].distance * 0.4f * (hit.point.x - pm.transform.position.x) / dis + pm.transform.position.x;
                    b = sm.skills[13].distance * 0.4f * (hit.point.z - pm.transform.position.z) / dis + pm.transform.position.z;
                    if (skillrangeObj != null)
                ...
                {
                    sc.id = 13;          //SkillCasting.cs id 값 저장.
        ...
        if (sc.GaugeDone[13]) //캐스팅 차징 끝나면
        {
            camshake.Shake(0.5f);
            ph.MP -= sm.skills[13].consumeMp;
            GameObject skillObj = Instantiate(skillPrefab, temp, Quaternion.Euler(-90, 0, 0));
            skillObj.transform.localScale = new Vector3(0.5f + sm.skills[13].explo * 0.002f, 0.5f + sm.skills[13].explo * 0.0015f, 0.5f + sm.skills[13].explo * 0.002f);
            Destroy(skillObj, sm.skills[13].sustaintime);
            isTimer = true;
            sc.GaugeDone[13] = false;
        }

        if (isTimer)
        {
            float cooltime = sm.skills[13].cooltime * (1 - 0.01f * status.Calcool);
            if (timer >= cooltime)
            {
